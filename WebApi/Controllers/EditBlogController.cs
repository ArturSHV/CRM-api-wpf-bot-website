...
namespace WebApi.Controllers
{
    public class EditBlogController : Controller
    {
        ...
        public EditBlogController(IWebHostEnvironment appEnvironment)
        {
        ...
        public async Task<IActionResult> Index([FromBody] RequestedData<Blog> requestedData, [FromServices] DataContext context)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result == true)
                {
                    ...
                    var checkImage = context.Blogs.FirstOrDefault(x => x.Image == requestedData.model.Image);
                    if (checkImage == null)
                    {
                    ...
                    try
                    {
                        Blog? a = context.Blogs.FirstOrDefault(x => x.Id == requestedData.model.Id);
                        ...
                        if (a != null)
                        {
                            ...
                            a.CreateDate = requestedData.model.CreateDate;
                            await context.SaveChangesAsync();

                            statusCode = 200;

                            response = new Response { ok = true, status_code = statusCode, result = new { description = "Блог успешно изменен" } };
                        }
                        else
                        {
                            statusCode = 404;
